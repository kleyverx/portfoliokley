...
(function () {
  ...
  const CONFIG = {
    ...
    hoverFillColor: 'rgba(191, 0, 255, 0.12)',
    speedPxPerSec: 1,
    direction: 'right',
    lineWidth: 1,
    maxDPR: 2,
  ...
  function drawAll(time) {
    ...
    for (const s of instances) {
      ...
      // Aplicar máscara radial: difuminar la grid hacia los bordes del canvas
      // El centro del gradiente sigue el desplazamiento de la grid para que el "pulso" o brillo se mueva con ella
      ctx.save();
      ctx.globalCompositeOperation = 'destination-out';
      
      // Ajustamos el centro del gradiente para que se mueva síncronamente con los cuadrados
      // Esto evita que la grid parezca deslizarse debajo de una linterna estática
      const cx = (width / 2) + s.offsetX;
      const cy = (height / 2) + s.offsetY;
      
      const rx = width * 0.55;
      const ry = height * 0.55;
      const radial = ctx.createRadialGradient(cx, cy, Math.min(rx, ry) * 0.15, cx, cy, Math.max(rx, ry));
      radial.addColorStop(0, 'rgba(0,0,0,0)');
      radial.addColorStop(0.55, 'rgba(0,0,0,0)');
      radial.addColorStop(1, 'rgba(0,0,0,1)');
      
      ctx.fillStyle = radial;
      // Llenamos todo el canvas con la máscara
